...
<?php $__env->startSection('page-title', 'Download Logs'); ?>
<?php $__env->startSection('page-subtitle', 'View and manage all download activity'); ?>

<?php $__env->startSection('content'); ?>

<div class="bg-gray-900 rounded-xl p-5 border border-gray-800 mb-6">
    <form method="GET" action="<?php echo e(route('admin.logs')); ?>" class="flex flex-wrap items-end gap-4">
        <div>
            ...
            <select name="platform" class="bg-gray-800 text-white rounded-lg px-3 py-2 text-sm border border-gray-700 focus:ring-2 focus:ring-indigo-500 focus:outline-none min-w-[140px]">
                ...
                <option value="pinterest" <?php echo e(request('platform') === 'pinterest' ? 'selected' : ''); ?>>Pinterest</option>
                <option value="youtube" <?php echo e(request('platform') === 'youtube' ? 'selected' : ''); ?>>YouTube</option>
            </select>
        </div>
        <div>
            ...
            <select name="status" class="bg-gray-800 text-white rounded-lg px-3 py-2 text-sm border border-gray-700 focus:ring-2 focus:ring-indigo-500 focus:outline-none min-w-[140px]">
                ...
                <option value="downloaded" <?php echo e(request('status') === 'downloaded' ? 'selected' : ''); ?>>Downloaded</option>
                <option value="fetched" <?php echo e(request('status') === 'fetched' ? 'selected' : ''); ?>>Fetched Only</option>
            </select>
        </div>
        <div>
        ...
        <div class="flex-1 min-w-[200px]">
            ...
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="URL, title, or IP address..."
        ...
        <div class="flex gap-2">
            ...
            <?php if(request()->hasAny(['platform', 'status', 'search', 'date_from', 'date_to'])): ?>
...
<div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
    ...
    <div class="px-5 py-3 border-b border-gray-800 flex items-center justify-between">
        <p class="text-sm text-gray-400">
            Showing <span class="text-white font-medium"><?php echo e($logs->firstItem() ?? 0); ?></span>–<span class="text-white font-medium"><?php echo e($logs->lastItem() ?? 0); ?></span>
            of <span class="text-white font-medium"><?php echo e(number_format($logs->total())); ?></span> entries
        </p>
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('admin.export', request()->query())); ?>"
            ...
            <tbody class="divide-y divide-gray-800">
                <?php $__empty_1 = true; $__currentLoopData = $logs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <tr class="hover:bg-gray-800/30 transition group">
                        <td class="px-5 py-3 text-gray-600 text-xs font-mono">#<?php echo e($log->id); ?></td>
                        <td class="px-5 py-3">
                            <span class="px-2.5 py-1 rounded-full text-[10px] font-semibold <?php echo e($badges[$log->platform] ?? 'bg-gray-700 text-gray-400'); ?>">
                                <?php echo e(ucfirst($log->platform)); ?>

                            </span>
                        </td>
                        <td class="px-5 py-3 max-w-xs">
                            <a href="<?php echo e($log->source_url); ?>" target="_blank" rel="noopener noreferrer" class="block hover:opacity-80 transition">
                                <?php if($log->video_title): ?>
                                    <p class="text-gray-200 text-xs truncate font-medium hover:text-indigo-400 transition" title="<?php echo e($log->video_title); ?>"><?php echo e($log->video_title); ?></p>
                                <?php endif; ?>
                                <p class="text-gray-600 text-[10px] truncate hover:text-indigo-400/60 transition" title="<?php echo e($log->source_url); ?>"><?php echo e($log->source_url); ?></p>
                            </a>
                        </td>
                        <td class="px-5 py-3 font-mono text-xs text-gray-400"><?php echo e($log->ip_address); ?></td>
                        <td class="px-5 py-3 text-xs text-gray-400"><?php echo e($browser); ?></td>
                        ...
                        <td class="px-5 py-3 text-xs text-gray-500 whitespace-nowrap">
                            <p><?php echo e($log->created_at->format('M d, Y')); ?></p>
                            <p class="text-gray-600"><?php echo e($log->created_at->format('h:i:s A')); ?></p>
                        </td>
                        <td class="px-5 py-3 text-center">
                            <form method="POST" action="<?php echo e(route('admin.logs.delete', $log->id)); ?>"
    ...
    <?php if($logs->hasPages()): ?>
        <div class="px-5 py-4 border-t border-gray-800 flex items-center justify-between">
            <p class="text-xs text-gray-600">Page <?php echo e($logs->currentPage()); ?> of <?php echo e($logs->lastPage()); ?></p>
            <div class="flex gap-1">
                ...
                <?php else: ?>
                    <a href="<?php echo e($logs->previousPageUrl()); ?>" class="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-lg text-xs transition">Previous</a>
                <?php endif; ?>
                <?php if($logs->hasMorePages()): ?>
                    <a href="<?php echo e($logs->nextPageUrl()); ?>" class="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-lg text-xs transition">Next</a>
                <?php else: ?>
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\tiktok-downloader\resources\views/admin/logs.blade.php ENDPATH**/ ?>
...
<head>
    ...
    <title>@yield('title', 'Admin Panel') — Video Downloader</title>
        ...
        <nav class="flex-1 p-3 space-y-1 overflow-y-auto scrollbar-thin">
            <a href="{{ route('admin.dashboard') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition
                      {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-600/20 text-indigo-400' : 'text-gray-400 hover:bg-sidebar-hover hover:text-white' }}">
            ...
            <a href="{{ route('admin.logs') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition
                      {{ request()->routeIs('admin.logs') ? 'bg-indigo-600/20 text-indigo-400' : 'text-gray-400 hover:bg-sidebar-hover hover:text-white' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.settings') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition
                      {{ request()->routeIs('admin.settings') ? 'bg-indigo-600/20 text-indigo-400' : 'text-gray-400 hover:bg-sidebar-hover hover:text-white' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <header class="sticky top-0 z-20 bg-gray-950/80 backdrop-blur-lg border-b border-gray-800 px-8 py-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">@yield('page-subtitle', 'Overview of your video downloader')</p>
                </div>
                <div class="flex items-center gap-4">
                    <span class="text-xs text-gray-500">{{ now()->format('M d, Y — h:i A') }}</span>
                </div>
            </div>
        </header>

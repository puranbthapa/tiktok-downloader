...
@section('page-subtitle', 'Real-time overview of downloads and visitors')
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
    <div class="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
        ...
        <p class="text-2xl font-bold text-white">{{ number_format($totalFetches) }}</p>
    ...
    <div class="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
        ...
        <p class="text-2xl font-bold text-green-400">{{ number_format($totalDownloads) }}</p>
    ...
    <div class="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
        ...
        <p class="text-2xl font-bold text-yellow-400">{{ $conversionRate }}%</p>
    ...
    <div class="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
        ...
        <p class="text-2xl font-bold text-indigo-400">{{ number_format($todayFetches) }}</p>
        <p class="text-xs text-gray-600 mt-1">{{ $todayDownloads }} downloaded</p>
    </div>
    ...
    <div class="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
        ...
        <p class="text-2xl font-bold text-purple-400">{{ number_format($uniqueVisitors) }}</p>
    ...
    <div class="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
        ...
        <p class="text-2xl font-bold text-pink-400">{{ $platformStats->count() }}</p>
                ...
                <div class="flex-1 flex flex-col items-center gap-1 group relative">
                    <div class="absolute bottom-full mb-2 hidden group-hover:flex flex-col items-center z-10">
                        <div class="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-xs whitespace-nowrap shadow-xl">
                            ...
                            <p class="text-indigo-400">{{ $day->fetches }} fetches</p>
                            <p class="text-green-400">{{ $day->downloads }} downloads</p>
                        </div>
                    </div>
                    <div class="w-full flex flex-col items-center gap-0.5" style="height: 180px;">
                        <div class="w-full flex items-end gap-0.5 h-full">
                            <div class="flex-1 bg-indigo-500/80 rounded-t-sm transition-all duration-300 hover:bg-indigo-400"
                                 style="height: {{ $maxVal > 0 ? round(($day->fetches / $maxVal) * 100) : 0 }}%; min-height: {{ $day->fetches > 0 ? '4px' : '0' }}"></div>
                            <div class="flex-1 bg-green-500/80 rounded-t-sm transition-all duration-300 hover:bg-green-400"
                                 style="height: {{ $maxVal > 0 ? round(($day->downloads / $maxVal) * 100) : 0 }}%; min-height: {{ $day->downloads > 0 ? '4px' : '0' }}"></div>
                        </div>
                    </div>
                    <span class="text-[10px] text-gray-600 mt-1">{{ \Carbon\Carbon::parse($day->date)->format('d') }}</span>
                </div>
            @endforeach
        </div>
    </div>
            ...
            <div class="mb-5 last:mb-0">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                        <div class="w-3 h-3 {{ $c['bg'] }} rounded-full"></div>
                        <span class="text-sm font-medium text-white capitalize">{{ $stat->platform }}</span>
                    </div>
                    <span class="text-xs text-gray-500">{{ $stat->total }} fetches &bull; {{ $stat->downloads }} DL</span>
                </div>
                <div class="w-full bg-gray-800 rounded-full h-2.5">
                    <div class="{{ $c['bg'] }} h-2.5 rounded-full transition-all duration-700" style="width: {{ $pct }}%"></div>
                </div>
                <div class="flex justify-between mt-1">
                    <span class="text-[10px] text-gray-600">{{ $pct }}% of total</span>
                    <span class="text-[10px] text-gray-600">{{ $dlPct }}% conversion</span>
                </div>
            </div>
        @empty
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-gray-900 rounded-xl p-6 border border-gray-800">
        ...
        @php $maxHour = $hourlyStats->max('total') ?: 1; @endphp
        <div class="flex items-end gap-0.5 h-32">
            @for($h = 0; $h < 24; $h++)
                @php $val = $hourlyStats[$h]->total ?? 0; @endphp
                <div class="flex-1 group relative">
                    <div class="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 hidden group-hover:block z-10">
                        <div class="bg-gray-800 border border-gray-700 rounded px-2 py-1 text-[10px] text-white whitespace-nowrap shadow-lg">
                            {{ str_pad($h, 2, '0', STR_PAD_LEFT) }}:00 — {{ $val }}
                        </div>
                    </div>
                    <div class="w-full bg-indigo-500/60 rounded-t-sm hover:bg-indigo-400 transition-all cursor-pointer"
                         style="height: {{ $maxHour > 0 ? max(round(($val / $maxHour) * 128), ($val > 0 ? 4 : 1)) : 1 }}px">
                    </div>
                </div>
            @endfor
        </div>
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($topUrls as $i => $url)
                        ...
                        <tr class="hover:bg-gray-800/50 transition">
                            ...
                            <td class="py-2.5">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium {{ $badges[$url->platform] ?? 'bg-gray-700 text-gray-400' }}">
                                    {{ ucfirst($url->platform) }}
                                </span>
                            </td>
                            <td class="py-2.5 max-w-xs">
                                <a href="{{ $url->source_url }}" target="_blank" rel="noopener noreferrer" class="block hover:opacity-80 transition">
                                    @if($url->video_title)
                                        <p class="text-gray-300 text-xs truncate hover:text-indigo-400 transition" title="{{ $url->video_title }}">{{ $url->video_title }}</p>
                                    @endif
                                    <p class="text-gray-600 text-[10px] truncate hover:text-indigo-400/60 transition" title="{{ $url->source_url }}">{{ $url->source_url }}</p>
                                </a>
                            </td>
                            <td class="py-2.5 text-right text-white font-medium text-xs">{{ $url->hits }}</td>
                            <td class="py-2.5 text-right text-green-400 text-xs">{{ $url->dl_count }}</td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="bg-gray-900 rounded-xl p-6 border border-gray-800">
        ...
        <div class="space-y-3">
            @forelse($browsers as $browser)
                @php $bpct = round(($browser->total / $browserTotal) * 100); @endphp
                <div>
                    <div class="flex justify-between text-sm mb-1">
                        <span class="text-gray-300">{{ $browser->name }}</span>
                        <span class="text-gray-500 text-xs">{{ $browser->total }} ({{ $bpct }}%)</span>
                    </div>
                    <div class="w-full bg-gray-800 rounded-full h-2">
                        <div class="{{ $browserColors[$browser->name] ?? 'bg-gray-500' }} h-2 rounded-full transition-all duration-500"
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($recentVisitors as $v)
                        <tr class="hover:bg-gray-800/50 transition">
                            <td class="py-2 font-mono text-xs text-gray-300">{{ $v->ip_address }}</td>
                            <td class="py-2 text-right">
                                <span class="px-2 py-0.5 bg-indigo-500/15 text-indigo-400 rounded-full text-xs font-medium">{{ $v->requests }}</span>
                            </td>
                            <td class="py-2 text-right text-gray-500 text-xs">{{ \Carbon\Carbon::parse($v->last_seen)->diffForHumans() }}</td>
                        </tr>
                    @empty
                        <tr>

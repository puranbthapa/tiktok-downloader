...
@section('page-subtitle', 'Admin panel configuration')
    ...
    <div class="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
            ...
            <div class="bg-gray-800/50 rounded-lg px-4 py-3">
                ...
                <p class="text-white font-medium capitalize">{{ app()->environment() }}</p>
            </div>
            <div class="bg-gray-800/50 rounded-lg px-4 py-3">
                <p class="text-gray-500 text-xs mb-1">Debug Mode</p>
                <p class="font-medium {{ config('app.debug') ? 'text-yellow-400' : 'text-green-400' }}">
                    {{ config('app.debug') ? 'Enabled' : 'Disabled' }}
                </p>
            </div>
            <div class="bg-gray-800/50 rounded-lg px-4 py-3">
                <p class="text-gray-500 text-xs mb-1">Session Driver</p>
                <p class="text-white font-medium capitalize">{{ config('session.driver') }}</p>
            </div>
            <div class="bg-gray-800/50 rounded-lg px-4 py-3">
                <p class="text-gray-500 text-xs mb-1">Database</p>
                <p class="text-white font-medium">{{ config('database.connections.' . config('database.default') . '.database') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-900 rounded-xl p-6 border border-red-900/30 mb-6">
        ...
        <div x-data="{ showConfirm: false }">
            ...
            <div x-show="showConfirm" x-cloak class="mt-3 bg-red-950/30 border border-red-900/30 rounded-lg p-4">
                ...
                <form method="POST" action="{{ route('admin.logs.clear') }}" class="flex gap-2">
                    @csrf
                    @method('DELETE')
